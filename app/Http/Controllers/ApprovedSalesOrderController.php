<?php

namespace App\Http\Controllers;

use Illuminate\Http\Request;
use App\Models\Requisition;
use App\Models\RequisitionDetail;
use App\Models\Distributor;
use Illuminate\Support\Facades\Auth;

class ApprovedSalesOrderController extends Controller
{
     public function index()
    {
        $user = auth()->user();
        $distributorId = $user->distributor;
        $allRequisitions = Requisition::where('operations_approved_status',1)->where('distributor_id', $distributorId)->get();
        return view('approved_sales_order.index',compact('allRequisitions'));
    }

public function getData(Request $request)
{
    $user = auth()->user();
    $distributorId = $user->distributor;

    if (!$distributorId) {
        return response()->json([
            "draw" => intval($request->input('draw')),
            "recordsTotal" => 0,
            "recordsFiltered" => 0,
            "data" => [],
        ]);
    }

    $columns = [
        0 => 'id',
        1 => 'requisition_id',
        2 => 'requisition_date',
        3 => 'name',
        4 => 'address',
        5 => 'mobile',
        6 => 'quantity',
        7 => 'amount',
        8 => 'rebate',
        9 => 'total_amount',
        10 => 'requisition_note',
        11 => 'operations_approved_note',
        12 => 'operations_approved_status',
    ];

    $limit = $request->input('length', 10);
    $start = $request->input('start', 0);
    $orderColumnIndex = $request->input('order.0.column', 0);
    $orderColumn = $columns[$orderColumnIndex] ?? 'id';
    $orderDir = $request->input('order.0.dir', 'asc');

    $query = Requisition::where('operations_approved_status', 1)
        ->where('distributor_id', $distributorId);

    // Filter: Requisition ID
    if ($request->requisition_id) {
        $query->where('requisition_id', $request->requisition_id);
    }

    // Filter: Date Range
    if ($request->start_date && $request->end_date) {
        $query->whereBetween('requisition_date', [$request->start_date, $request->end_date]);
    } elseif ($request->start_date) {
        $query->whereDate('requisition_date', '>=', $request->start_date);
    } elseif ($request->end_date) {
        $query->whereDate('requisition_date', '<=', $request->end_date);
    }

    // Global Search
    if ($search = $request->input('search.value')) {
        $query->where(function ($q) use ($search) {
            $q->where('requisition_id', 'like', "%{$search}%")
              ->orWhere('requisition_date', 'like', "%{$search}%")
              ->orWhere('name', 'like', "%{$search}%")
              ->orWhere('address', 'like', "%{$search}%")
              ->orWhere('mobile', 'like', "%{$search}%")
              ->orWhere('requisition_note', 'like', "%{$search}%")
              ->orWhere('operations_approved_note', 'like', "%{$search}%");
        });
    }

    // Count records
    $totalFiltered = $query->count();
    $totalData = Requisition::where('operations_approved_status', 1)
                            ->where('distributor_id', $distributorId)
                            ->count();

    // Fetch paginated and ordered data
    $requisitions = $query->orderBy($orderColumn, $orderDir)
                          ->offset($start)
                          ->limit($limit)
                          ->get();

    // Format data
    $data = [];
    $serial = $start + 1;

    foreach ($requisitions as $req) {
        $data[] = [
            'sl' => $serial++,
            'requisition_id' => $req->requisition_id,
            'requisition_date' => $req->requisition_date,
            'name' => $req->name,
            'address' => $req->address,
            'mobile' => $req->mobile,
            'quantity' => $req->quantity,
            'amount' => number_format($req->amount, 2),
            'rebate' => number_format($req->rebate, 2),
            'total_amount' => number_format($req->total_amount, 2),
            'status' => '<span class="badge badge-success">Approved</span>',
            'actions' => '
                <a href="' . route('approved-sales-order.show', $req->id) . '" class="btn btn-sm btn-info" title="Details">
                    <i class="fas fa-eye"></i>
                </a>',
        ];
    }

    return response()->json([
        "draw" => intval($request->input('draw')),
        "recordsTotal" => $totalData,
        "recordsFiltered" => $totalFiltered,
        "data" => $data,
    ]);
}


 public function show($id)
    {
         $requisition = Requisition::findOrFail($id);
    
        // Get related details from RequisitionDetail model
        $details = RequisitionDetail::where('requisition_id', $requisition->requisition_id)->get();
        $distributor = Distributor::where('id',$requisition->distributor_id)->first();
    
        return view('approved_sales_order.show', compact('requisition', 'details','distributor'));
        
    }


}
