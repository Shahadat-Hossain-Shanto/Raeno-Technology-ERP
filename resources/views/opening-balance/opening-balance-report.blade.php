...
@section('title', 'Opening Balance Report')
                    ...
                    <div class="col-lg-12">
                        ...
                        <div class="card card-primary">
                            <div class="card-header">
                                <h5 class="m-0"><strong><i class="fas fa-file-contract"></i>Opening Balance Report</strong></h5>
                            </div>
                            <div class="card-body">
                                <div id="form_div">
                                    <form id="openingbalanceReportForm" >
                                        <div class="form-row">
                                ...
                                <div id="tablePart" class="table-responsive" >
                                    <hr>
                                    <h5 class="pb-3"><strong><u>Opening Balance History</u></strong></h5>
                                    {{-- <h6 class='float-right'><strong>Balance : </strong> &nbsp;&nbsp;&nbsp;<span id='debit'></span>Dr &nbsp;&nbsp;&nbsp;<span id='credit'></span>Cr &nbsp;&nbsp;&nbsp;</h6> --}}
                                    <table id="opening_balance_table" class="display" style="100%!important">
                                        <thead>
                                            <tr>
                                                <th>#</th>
...
@section('script')
    ...
    <script>
        ...
        $(document).ready(function() {
            $('#openingbalanceReportForm').on('submit', function(e) {
                var transaction_id = $('#transaction_id').val();
                var startdate=$('#startdate').val();
                var enddate=$('#enddate').val();
                var voucher = 'Opening Balance';
               ...
               if (transaction_id !=''|| startdate.length != 0 && enddate.length != 0) {
                ...
                $.ajax({
                    type: "get",
                    url: "/opening-balance-report/"+transaction_id+"/"+startdate+"/"+enddate+"/"+voucher,

                    success: function(response) {
                        // console.log(response.opening_balance);
                        var totalDebit=0;
                        var totalCredit=0;
                        response.opening_balance.forEach(openingbalance => {
                            totalCredit = totalCredit+openingbalance.credit;
                            totalDebit = totalDebit+openingbalance.debit;
                        });
                        $('#debit').text(totalDebit.toFixed(2));
                        $('#credit').text(totalCredit.toFixed(2));
                        i=1;
                        $('#tablePart').show();
                        onChangeDataTable(response.opening_balance,i)
...
$('#opening_balance_table').DataTable().clear().destroy()

var t = $('#opening_balance_table').DataTable({
