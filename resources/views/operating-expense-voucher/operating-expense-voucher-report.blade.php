...
@section('title', 'Operating Expense Voucher Report')
                    ...
                    <div class="col-lg-12">
                        ...
                        <div class="card card-primary">
                            <div class="card-header">
                                <h5 class="m-0"><strong><i class="fas fa-file-contract"></i>Operating Expense Voucher Report</strong></h5>
                            </div>
                            <div class="card-body">
                                <div id="form_div">
                                    <form id="expenseVoucherReportForm" >
                                        <div class="form-row">
                                ...
                                <div id="tablePart" >
                                    <hr>
                                    <h5 class="pb-3"><strong><u>Expense Voucher History</u></strong></h5>
                                    {{-- <h6 class='float-right'><strong>Balance : </strong> &nbsp;&nbsp;&nbsp;<span id='debit'></span>Dr &nbsp;&nbsp;&nbsp;<span id='credit'></span>Cr &nbsp;&nbsp;&nbsp;</h6> --}}
                                    <table id="expense_voucher_table" class="display" style="100%!important">
                                        <thead>
                                            <tr>
                                                <th>#</th>
...
@section('script')
    ...
    <script>
        ...
        $(document).ready(function() {
            $('#expenseVoucherReportForm').on('submit', function(e) {
                var transaction_id = $('#transaction_id').val();
                var startdate=$('#startdate').val();
                var enddate=$('#enddate').val();
                var voucher = 'Operating Expense';
               ...
               if (transaction_id !=''|| startdate.length != 0 && enddate.length != 0) {
                ...
                $.ajax({
                    type: "get",
                    url: "/operating-expense-voucher-data/"+transaction_id+"/"+startdate+"/"+enddate+"/"+voucher,

                    success: function(response) {
                        // console.log(response.expense);
                        var totalDebit=0;
                        var totalCredit=0;
                        response.expense.forEach(Expense => {
                            totalCredit = totalCredit+Expense.credit;
                            totalDebit = totalDebit+Expense.debit;
                        });
                        $('#debit').text(totalDebit.toFixed(2));
                        $('#credit').text(totalCredit.toFixed(2));
                        i=1;
                        $('#tablePart').show();
                        onChangeDataTable(response.expense,i)
...
$('#expense_voucher_table').DataTable().clear().destroy()

var t = $('#expense_voucher_table').DataTable({
