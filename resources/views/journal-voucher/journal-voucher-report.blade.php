...
@section('title', 'Adjustment Voucher Report')
                    ...
                    <div class="col-lg-12">
                        ...
                        <div class="card card-primary">
                            <div class="card-header">
                                <h5 class="m-0"><strong><i class="fas fa-file-contract"></i>Adjustment Voucher Report</strong></h5>
                            </div>
                            <div class="card-body">
                                <div id="form_div">
                                    <form id="journalVoucherReportForm" >
                                        <div class="form-row">
                                ...
                                <div id="tablePart" class="table-responsive" >
                                    <hr>
                                    <h5 class="pb-3"><strong><u>Adjustment Voucher History</u></strong></h5>
                                    {{-- <h6 class='float-right'><strong>Balance : </strong> &nbsp;&nbsp;&nbsp;<span id='debit'></span>Dr &nbsp;&nbsp;&nbsp;<span id='credit'></span>Cr &nbsp;&nbsp;&nbsp;</h6> --}}
                                    <table id="journal_voucher_table" class="display" style="100%!important">
                                        <thead>
                                            <tr>
                                                <th>#</th>
...
@section('script')
    ...
    <script>
        ...
        $(document).ready(function() {
            $('#journalVoucherReportForm').on('submit', function(e) {
                var transaction_id = $('#transaction_id').val();
                var startdate=$('#startdate').val();
                var enddate=$('#enddate').val();
                var voucher = 'Adjustment Voucher';
               ...
               if (transaction_id !=''|| startdate.length != 0 && enddate.length != 0) {
                ...
                $.ajax({
                    type: "get",
                    url: "/journal-voucher-report/"+transaction_id+"/"+startdate+"/"+enddate+"/"+voucher,

                    success: function(response) {
                        // console.log(response.journal_vouchers);
                        var totalDebit=0;
                        var totalCredit=0;
                        response.journal_vouchers.forEach(journal_voucher => {
                            totalCredit = totalCredit+journal_voucher.credit;
                            totalDebit = totalDebit+journal_voucher.debit;
                        });
                        $('#debit').text(totalDebit.toFixed(2));
                        $('#credit').text(totalCredit.toFixed(2));
                        i=1;
                        $('#tablePart').show();
                        onChangeDataTable(response.journal_vouchers,i)
...
$('#journal_voucher_table').DataTable().clear().destroy()

var t = $('#journal_voucher_table').DataTable({
