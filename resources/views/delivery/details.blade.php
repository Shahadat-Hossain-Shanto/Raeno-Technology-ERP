...
<div class="content-wrapper">
    ...
    <div id="invoiceArea" class="bg-white border rounded shadow-sm container-fluid px-5 py-5" style="max-width: 1100px; margin: 0 auto;">
        ...
        <div class="text-center mb-3">
            ...
            <p class="text-muted">Delivery Slip</p>
        </div>
        ...
        {{-- Footer --}}
        <div class="d-flex justify-content-between align-items-center text-muted border-top pt-2 mt-4">
            <div>Thank you for doing business with Raeno Technology.</div>
            <div class="text-end">System Generated</div>
        </div>
    </div>

    {{-- SECOND PRINT BUTTON and AREA --}}
<div class="container px-0 d-print-none mt-5 mb-2">
    <div class="row">
        <div class="col-12 d-flex flex-wrap align-items-center position-relative" style="padding-left: 30px;">
            <button onclick="printArea('invoiceArea2')" class="btn btn-sm btn-outline-success mt-3 mt-md-0">
                <i class="fas fa-print"></i> Print Total Invoice
            </button>
            <h3 class="mx-auto w-100 text-center mt-2 mt-md-0" style="position: static; transform: none; margin-top: 0;">
                Total Delivery Invoice
            </h3>
        </div>
    </div>
</div>

    <div id="invoiceArea2" class="bg-white border rounded shadow-sm container-fluid px-5 py-5" style="max-width: 1100px; margin: 0 auto;">
        <div class="text-center mb-3">
            <h2 class="fw-bold text-uppercase mb-1" style="font-size: 22px;">Raeno Technology</h2>
            <p class="text-muted">Delivery Slip</p>
        </div>
        ...
        <div class="row  mb-3">
            ...
            <div class="col-6 text-end">
                <p class="mb-1"><strong>Order Date:</strong> {{ $order->requisition_date ? \Carbon\Carbon::parse($order->requisition_date)->format('d M Y') : 'N/A' }}</p>
                <p class="mb-1"><strong>Approved Date:</strong> {{ $order->sales_approved_date ? \Carbon\Carbon::parse($order->sales_approved_date)->format('d M Y') : 'N/A' }}</p>
                <p class="mb-1"><strong>Delivery Date:</strong> {{ $delivery->created_at ? $delivery->created_at->format('d M Y') : 'N/A' }}</p>
            </div>
            <div class="col-12">
                {{-- <p class="mb-1 border"><strong>Delivery Medium:</strong> {{ $delivery->medium ?? 'N/A' }}</p> --}}
            </div>
        </div>
        ...
        <div class="table-responsive">
            <table class="table table-sm table-bordered">
                ...
                <tbody>
                    @php
                        $grouped = $deliveryAll->groupBy(fn($item) => $item->product_name . '|' . $item->model . '|' . $item->variant);
                    @endphp
                        @forelse($grouped as $key => $items)
                            @php
                                [$productName, $model, $variant] = explode('|', $key);
                ...
                @if($deliveryAll->count())
                    <tfoot>
                        <tr class="table-primary fw-semibold">
                            <td colspan="3" class="text-end">Total Quantity:</td>
                            <td class="text-end">{{ $deliveryAll->sum('quantity') }} {{ Str::plural('unit', $deliveryAll->sum('quantity')) }}</td>
                        </tr>
                    </tfoot>
                @endif
            </table>
