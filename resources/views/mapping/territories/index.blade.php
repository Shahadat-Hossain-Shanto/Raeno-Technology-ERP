...
@section('title', 'RAENO :: Territory List')
        ...
        <div class="">
            <div class="card card-primary">
                <div class="card-header d-flex justify-content-between" style="height: 49px;">
                    <h5><strong><i class="fas fa-map-marker-alt"></i> Territory List</strong></h5>
                </div>
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table id="territoryTable" class="table table-bordered table-striped dt-responsive nowrap" style="width:100%">
                            <thead>
                                <tr>
                                    <th>#</th>
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="userMapModalLabel">TSM Users</h5>
...
<script>
        ...
        $('#territoryTable').DataTable({
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function(row) {
                        return `
                            <button class="btn btn-info btn-sm user-map-btn" data-territory-id="${row.id}">
                ...
                },
                {
                    ...
                    render: function(row) {
                        return `
                            <a href="/territories/${row.id}/edit" class="btn btn-warning btn-sm"><i class="fas fa-edit"></i></a>
                            <form action="/territories/${row.id}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
    ...
    $(document).on('click', '.user-map-btn', function () {
        const territoryId = $(this).data('territory-id');

        $('#userMapContent').html('Loading...');
        $('#userMapModal').modal('show');

        $.ajax({
            url: `/territories/${territoryId}/users`,
            method: 'GET',
            success: function (response) {
                let html = `
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <input type="text" class="form-control w-50" id="userSearchInput" placeholder="🔍 Search users...">
                        <button class="btn btn-success ms-3" id="bulkAssignBtn" data-territory-id="${territoryId}">
                ...
                if (response.users.length > 0) {
                    ...
                    html += '<tr><td colspan="6" class="text-center">No RSM users found for this territory.</td></tr>';
                }
    ...
    $(document).on('click', '#bulkAssignBtn', function () {
        ...
        $.ajax({
            url: '/users/assign-territory-bulk',
            method: 'POST',
            data: {
                user_ids: userIds,
